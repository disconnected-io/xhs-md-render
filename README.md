...
- **封面模板**：10 套内置封面设计（米白·红线、米白·暖橙、米白·青叶、米白·墨点、暗夜·绯红、暗夜·金辉、暗夜·紫雾、暗夜·青蓝、纯白·炽红、纯白·科技蓝）
- **手机适配**：大字号（正文 36px），手机阅读舒适
- **多样主题**：10 套封面模板，3 种配色风格（米白、暗夜、纯白），正文自动匹配明暗
...
编辑 `render.js` 顶部的 CSS 变量和封面 HTML 来定制样式。`preview.js` 中的 `templates` 数组包含 10 套模板配色和几何装饰。

## 模板

10 套封面模板，3 种配色风格。点击 `preview.html` 选择后复制命令，或直接 `--cover=` 指定：
